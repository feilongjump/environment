...
- 2026-09-04 通道③→② 迁移启动并完成阶段 1+2（V21 先行，moni 待服务商切换明朗）。已完成：compose.yml 入库并登记 include；V21 解除 9db6621 锁定（预建 otb/flowstock `api/.env` 空壳后 pull 至 450d3e5，compose 解析通过，postgres 无扰动）；V21 布局 `projects/retail-integration/api/`（预置当日 binary + config.yaml，DB host 已改 `postgres`）；建 deploy 用户（docker 组）+ restrict CI key，secrets `V21_SSH_HOST/USER/KEY` 已入 GitHub；项目仓库加 `/healthz` 探针 + deploy.yml（push main 仅构建；dispatch 必选目标；含 systemd 未切换闸门防 8090 撞口）。事故记录：pwsh→ssh 双层引号绞碎曾在 V21 `.env` 留下毒行 `n#` 致 compose 解析失败，已清除——**复杂远程脚本一律走"本地写文件→scp→bash"**。**待办（阶段 3 切换，等用户确认）**：EZR 生产参数替换（现 `V21_test`）→ systemd stop/disable → compose up → 验证 → /opt 与 unit 保留一周后清。
